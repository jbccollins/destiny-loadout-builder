	...
	for (const item of items) {
		...
		try {
			createdItem = makeItem(
				...
				owner,
				mergedCollectibles,
				null, //uninstancedItemObjectives,
				profileRecords
			);
		} catch (e) {
			errorLog('d2-stores', 'Error processing item', item, e);
			console.error('Processing Dim item', e);
		}
		if (
			createdItem !== null &&
			...
			if (
				...
				console.error('setting store hadErrors');
				throw (
					(new Error('setting store hadErrors'),
					{
...
export function makeItem(
	...
	// Missing definition?
	if (!itemDef) {
		console.warn('missing definition');
	...
	if (itemDef.redacted) {
		...
		const collectibleDef = collectiblesByItemHash(defs.Collectible.getAll())[
			item.itemHash
		];
		if (collectibleDef) {
	...
	if (
		...
		(owner?.isVault || item.location === ItemLocation.Vault)
	) {
	...
	let powerCap =
		(powerCapHash && defs.PowerCap.get(powerCapHash).powerCap) || null;
	...
	if (
		...
		const questLineItem = defs.InventoryItem.get(
			itemDef.objectives.questlineItemHash
		);
		if (
			questLineItem &&
			questLineItem.displayProperties.name !== itemDef.displayProperties.name
		) {
			typeName = itemDef.displayProperties.name;
	...
	const createdItem: DimItem = {
		...
		trackable: Boolean(
			item.itemInstanceId && itemDef.objectives?.questlineItemHash
		),
		tracked: Boolean(item.state & ItemState.Tracked),
		locked: Boolean(item.state & ItemState.Locked),
		masterwork:
			Boolean(item.state & ItemState.Masterwork) && itemType !== 'Class',
		...
		ammoType: itemDef.equippingBlock
			? itemDef.equippingBlock.ammoType
			: DestinyAmmunitionType.None,
		source,
		collectibleState,
		collectibleHash,
		...
		availableMetricCategoryNodeHashes:
			itemDef.metrics?.availableMetricCategoryNodeHashes,
	...
	try {
		...
		errorLog(
			'd2-stores',
			`Error building sockets for ${createdItem.name}`,
			item,
			itemDef,
			e
		);
		console.error('Sockets', e, { itemHash: item.itemHash });
	}

	createdItem.wishListEnabled = Boolean(
		createdItem.bucket.inWeapons && createdItem.sockets
	);
	...
	try {
		...
		errorLog(
			'd2-stores',
			`Error building stats for ${createdItem.name}`,
			item,
			itemDef,
			e
		);
		console.error('Stats', e, { itemHash: item.itemHash });
	}
	...
	// 		createdItem.percentComplete = _.sumBy(
	...
	// 					if (
	// 						checkTrialsPassage &&
	// 						!isWinsObjective(objective.objectiveHash)
	// 					) {
	// 						return 0;
	// 					}
	// 					return (
	// 						Math.min(
	// 							1,
	// 							(objective.progress || 0) / objective.completionValue
	...
	if (extendedBreaker[createdItem.hash]) {
		createdItem.breakerType = defs.BreakerType.get(
			extendedBreaker[createdItem.hash]
		);
	}
	...
	if (
		createdItem.itemCategoryHashes.includes(
			ItemCategoryHashes.LinearFusionRifles
		)
	) {
		const fusionRifleLocation = createdItem.itemCategoryHashes.indexOf(
			ItemCategoryHashes.FusionRifle
		);
		fusionRifleLocation !== -1 &&
			createdItem.itemCategoryHashes.splice(fusionRifleLocation, 1);
	}
	...
	createdItem.infusable =
		createdItem.infusionFuel && isLegendaryOrBetter(createdItem);
	...
	try {
		...
		console.error('MasterworkInfo', e, { itemHash: item.itemHash });
	}
	...
	try {
		...
		errorLog(
			'd2-stores',
			`Error building Quest info for ${createdItem.name}`,
			item,
			itemDef,
			e
		);
		console.error('Quest', e, { itemHash: item.itemHash });
	}

	if (
		createdItem.primaryStat &&
		lightStats.includes(createdItem.primaryStat.statHash)
	) {
...
function buildPursuitInfo(
	...
	const rewards = itemDef.value
		? itemDef.value.itemValue.filter((v) => v.itemHash)
		: [];
	if (rewards.length) {
		createdItem.pursuit = {
