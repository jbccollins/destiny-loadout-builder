import { armorStats } from '@dlb/dim/search/d2-known-values';
import {
...
export function makeVault(): DimStore {
	return {
		...
		current: false,
		className: 'Bucket.Vault',
		genderName: '',
		lastPlayed: new Date(-1),
		icon: 'vault-icon', //vaultIcon,
		background: 'vault-background', // vaultBackground,
		items: [],
		isVault: true,
	...
	if (titleRecordDef.titleInfo.gildingTrackingRecordHash) {
		const gildedRecord =
			profileRecords?.records[
				titleRecordDef.titleInfo.gildingTrackingRecordHash
			];
		...
		isGildedForCurrentSeason = Boolean(
			gildedRecord &&
				!(gildedRecord.state & DestinyRecordState.ObjectiveNotCompleted)
		);
	}
