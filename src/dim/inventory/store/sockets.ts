...
export function buildSockets(
	...
	const reusablePlugData =
		(item.itemInstanceId &&
			itemComponents?.reusablePlugs?.data?.[item.itemInstanceId]?.plugs) ||
		undefined;
	const plugObjectivesData =
		(item.itemInstanceId &&
			itemComponents?.plugObjectives?.data?.[item.itemInstanceId]
				?.objectivesPerPlug) ||
		undefined;
	if (socketData) {
	...
	if (!sockets && itemDef.sockets) {
		...
		if (
			...
			!socketData?.[item.itemInstanceId]
		) {
...
function filterReusablePlug(reusablePlug: DimPlug) {
	...
	return (
		...
		!reusablePlug.plugDef.plug?.plugCategoryIdentifier.includes(
			'masterworks.stat'
		)
	);
}
...
function buildDefinedSocket(
	...
	const socketTypeDef =
		socketDef.socketTypeHash &&
		defs.SocketType.get(socketDef.socketTypeHash, forThisItem);
	if (!socketTypeDef) {
		return undefined;
	}
	const socketCategoryDef = defs.SocketCategory.get(
	...
	function addCraftingReqs(
		plugEntry: DestinyItemSocketEntryPlugItemRandomizedDefinition
	) {
		if (
	...
	if (isPerk) {
		if (socketDef.reusablePlugSetHash) {
			const plugSet = defs.PlugSet.get(
			...
			if (plugSet) {
				...
				for (const randomPlug of plugSet.reusablePlugItems) {
					...
					if (
						!existing ||
						(!existing.currentlyCanRoll && randomPlug.currentlyCanRoll)
					) {
	...
	if (
		...
		!reusablePlugs.find(
			(rp) => rp.plugDef.hash === socketDef.singleInitialItemHash
		)
	) {
	...
	if (
		...
		(socketCategoryDef.hash === SocketCategoryHashes.IntrinsicTraits ||
			...
			plugOptions[0].plugDef.plug.plugCategoryHash ===
	...
	return {
		...
		hasRandomizedPlugItems:
			Boolean(socketDef.randomizedPlugSetHash) ||
			socketTypeDef.alwaysRandomizeSockets,
		isPerk,
		isReusable,
...
function addPlugOption(
	...
	if (built && filterReusablePlug(built)) {
		if (plug && built.plugDef.hash === plug.plugDef.hash) {
			...
			if (
				!built.plugDef.itemCategoryHashes?.includes(
...
function buildSocket(
	...
	const socketTypeDef = defs.SocketType.get(
		socketDef.socketTypeHash,
		forThisItem
	);
	if (!socketTypeDef) {
	...
	return {
		...
		plugSet,
		curatedRoll,
		emptyPlugItemHash: findEmptyPlug(
			socketDef,
			socketTypeDef,
			plugSet,
			reusablePlugs
 ...
 */
function buildCachedDimPlugSet(
	defs: D2ManifestDefinitions,
	plugSetHash: number
): DimPlugSet {
