...
export function buildStats(
	...
	return investmentStats.length
		? investmentStats.sort(compareBy((s) => s.sort))
		: null;
}
...
function buildInvestmentStats(
	...
	for (const itemStat of itemStats) {
		...
		if (
			!itemStat ||
			!shouldShowStat(itemDef, statHash, statDisplaysByStatHash)
		) {
			continue;
		}
...
function applyPlugsToStats(
	...
	for (const [affectsBase, socketList] of socketLists) {
		for (const socket of socketList) {
			...
			for (const pluggedInvestmentStat of socket.plugged.plugDef
				.investmentStats) {
...
function attachPlugStats(
	...
	if (activePlug) {
		...
		for (const plugInvestmentStat of activePlug.plugDef.investmentStats) {
			...
			const statDisplay =
				statDisplaysByStatHash[plugInvestmentStat.statTypeHash];

			if (itemStat) {
				const baseInvestmentStat = itemStat.investmentValue - plugStatValue;
				baseItemInvestmentStats[plugInvestmentStat.statTypeHash] =
					baseInvestmentStat;
				...
				if (statDisplay) {
					...
					const valueWithoutPerk = Math.min(
						baseInvestmentStat,
...
function customStat(
	...
	const customStatDef = null; // = settingsSelector(reduxStore.getState())
	// 	.customTotalStatsByClass[destinyClass];

	if (
		!customStatDef ||
		customStatDef.length === 0 ||
		customStatDef.length === 6
	) {
		return undefined;
	}
...
export function keyByStatHash(
	stats: DestinyStatDisplayDefinition[]
): StatDisplayLookup;
export function keyByStatHash(
	stats: (DimStat | DestinyStatDisplayDefinition)[]
): {
