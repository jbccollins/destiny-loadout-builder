 ...
 */
export const getVault = (stores: DimStore[]): DimStore | undefined =>
 ...
 */
export const findItemsByBucket = (
	store: DimStore,
	bucketId: number
): DimItem[] => itemsByBucket(store)[bucketId] ?? emptyArray();

/** Get the bonus power from the Seasonal Artifact */
export function getArtifactBonus(store: DimStore) {
	const artifact = findItemsByBucket(store, BucketHashes.SeasonalArtifact).find(
		(i) => i.equipped
	);
 ...
 */
export function spaceLeftForItem(
	store: DimStore,
	item: DimItem,
	stores: DimStore[]
) {
...
export function potentialSpaceLeftForItem(
	...
	if (store.isVault) {
		...
		occupiedStacks = item.bucket.vaultBucket
			? count(store.items, (i) =>
					Boolean(i.bucket.vaultBucket?.hash === vaultBucket.hash)
			  )
			: 0;
	} else {
	...
	if (item.uniqueStack) {
		...
		const checkStore =
			item.bucket.accountWide && !store.isVault
