...
import { getFirstSocketByCategoryHash } from '@dlb/dim/utils/socket-utils';
import { DamageType, DestinyClass } from 'bungie-api-ts-no-const-enum/destiny2';
...
export const getSubclassPlugCategories = memoizeOne(
	(defs: D2ManifestDefinitions) => {
		const results = new Map<PlugCategoryHashes, SubclassPlugCategory>();
		for (const [itemHashStr, subclassInfo] of Object.entries(
			subclassInfoByHash
		)) {
...
const baseImagesByDamageType: Partial<Record<DamageType, StaticImageData>> = {
...
export function getSubclassIconInfo(
	...
	if (base && item.sockets) {
		const superSocket = getFirstSocketByCategoryHash(
			item.sockets,
