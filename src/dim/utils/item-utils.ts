...
const modMetadataBySocketTypeHash = objectifyArray(
	modSocketMetadata,
	'socketTypeHashes'
);
...
export const getSpecialtySocketMetadatas = (
	item?: DimItem
 ...
 */
export const getInterestingSocketMetadatas = (
	item?: DimItem
): ModSocketMetadata[] | undefined => {
...
export function itemCanBeEquippedBy(
	...
	return itemCanBeEquippedByStoreId(
		item,
		store.id,
		store.classType,
		allowPostmaster
...
function isEnabledKillTrackerSocket(socket: DimSocket) {
	return (
		(socket.plugged?.plugObjectives[0]?.objectiveHash ?? 0) in
		killTrackerObjectivesByHash
	);
}
...
export const getItemKillTrackerInfo = (
	item: DimItem
): KillTracker | undefined =>
...
// 			(item.classified ||
// 				roi ||
// 				_.intersection(d1YearSourceHashes.year3, item.sourceHashes).length)
// 		) {
// 			year = 3;
// 		}
		...
		return Boolean(
			item.sockets?.allSockets.some(
				(s) =>
					...
					s.plugged?.plugDef.plug.energyCost?.energyType ===
						DestinyEnergyType.Arc
			) ||
				modsOnOtherItems?.some(
					(plugDef) =>
						modTypeTagByPlugCategoryHash[plugDef.plug.plugCategoryHash] ===
							'chargedwithlight' &&
	...
	if (
		...
		return (
			(item.classType === DestinyClass.Hunter &&
				statHash === StatHashes.Mobility) ||
			(item.classType === DestinyClass.Titan &&
				statHash === StatHashes.Resilience) ||
			(item.classType === DestinyClass.Warlock &&
