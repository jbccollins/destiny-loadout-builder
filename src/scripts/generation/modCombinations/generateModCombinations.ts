...
function generateModCombinations(): GenericRequiredModCombo[] {
  ...
  for (let numArtificeMods = 0; numArtificeMods <= NUM_ARMOR_PIECES; numArtificeMods++) {
    for (let numMajorMods = 0; numMajorMods <= NUM_ARMOR_PIECES; numMajorMods++) {
      for (let numMinorMods = 0; numMinorMods <= NUM_ARMOR_PIECES; numMinorMods++) {
        if (numMajorMods + numMinorMods <= NUM_ARMOR_PIECES) {
          ...
          result.push({ numArtificeMods, numMajorMods, numMinorMods, exactStatPoints });
        }
      }
    }
  }
...
function findOptimalCombinations(combinations: GenericRequiredModCombo[], targetValue: number): GenericRequiredModCombo[] {
  // Filter combinations to only those that meet the target value
  const validCombinations = combinations.filter(c => c.exactStatPoints >= targetValue);
  ...
  for (let i = 0; i < validCombinations.length; i++) {
    ...
    for (let j = 0; j < validCombinations.length; j++) {
      if (i !== j && isMoreOptimal(validCombinations[j], validCombinations[i])) {
        isDominated = true;
        break;
      }
    }
