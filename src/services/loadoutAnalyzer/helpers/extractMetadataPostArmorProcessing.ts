...
export default function extractMetadataPostArmorProcessing(
	params: ExtractMetadataPostArmorProcessingParams
): ExtractMetadataPostArmorProcessingOutput {
	const { processedArmor, loadout, preArmorProcessingMetadata, modIdList } =
		params;
	const hasResults = processedArmor.items.length > 0;
	const hasMissingArmor = loadout.armor.length < 5;

	const metadata: GetLoadoutsThatCanBeOptimizedProgressMetadata = cloneDeep(
		preArmorProcessingMetadata
	);
