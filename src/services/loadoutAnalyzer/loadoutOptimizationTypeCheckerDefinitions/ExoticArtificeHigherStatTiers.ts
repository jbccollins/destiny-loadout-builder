...
const checker: LoadoutOptimizationTypeChecker = (params) => {
  const meetsOptimizationCriteria = higherStatTiersChecker(params).meetsOptimizationCriteria;
  return {
    meetsOptimizationCriteria,
    shortCircuit: false,
    metadataOverrides: meetsOptimizationCriteria ? {
      maxPossibleExoticArtificeDesiredStatTiers: { ...params.metadata.maxPossibleDesiredStatTiers }
    } : {}
  };
};
