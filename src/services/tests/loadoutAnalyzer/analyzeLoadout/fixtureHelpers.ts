...
const getBaseArmorItems = (): ArmorItem[] => {
	const armorItems: ArmorItem[] = [Head, Arm, Chest, Leg, ClassItem];
	return armorItems;
};
...
const getBaseAllClassItemMetadata =
	(): DestinyClassToAllClassItemMetadataMapping => {
...
const getBaseAvailableExoticArmor = (): AvailableExoticArmor => {
	...
	availableExoticArmor[EDestinyClassId.Hunter][EArmorSlotId.Head].push(
		wormhusk
	);
...
const getBaseGetLoadoutsThatCanBeOptimizedProgressMetadata =
	(): GetLoadoutsThatCanBeOptimizedProgressMetadata => {
		return {
